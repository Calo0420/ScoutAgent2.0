...
def _connect(host: str, username: str, key_path: str, password: str, port: int) -> paramiko.SSHClient:
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    kwargs = {"hostname": host, "username": username, "port": port, "timeout": 10}
    if key_path:
...
def check_cis_benchmarks(host: str, username: str, key_path: str = None, password: str = None, port: int = 22) -> dict:
    ...
    try:
        ...
        return {
            "host":       host,
            "controls":   controls,
            "score":      f"{passed_count}/{len(controls)}",
            "pct":        round(passed_count / len(controls) * 100),
            "risk_level": "LOW" if passed_count >= 8 else "MEDIUM" if passed_count >= 5 else "HIGH",
        }
    finally:
        client.close()
